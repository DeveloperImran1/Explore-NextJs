   ...
   const handler=() => {
    ...
    return (
        ...
        <ul className="flex items-center justify-between gap-6 text-slate-900">
           ...
           {
            links.map((link, index) =>  <li key={index} className="cursor-pointer  rounded-full  px-6 py-2 text-white hover:bg-sky-600"><Link className={pathName === link.path && 'text-red-600'} href={link.path}>{link.title}</Link></li> )
           }
        </ul>
        <div>
         <li onClick={handler} className="cursor-pointer flex gap-2 list-none rounded-full px-6 py-2 text-white hover:bg-sky-600">
